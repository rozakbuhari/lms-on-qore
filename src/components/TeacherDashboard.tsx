...
function TeacherDashboard() {
  ...
  const { user } = useUser();
  const { data: teacher } = qoreContext.views.allTeachers.useGetRow(
    user?.teachers?.nodes[0]?.id
  );
  const { insertRow } = qoreContext.views.allCourses.useInsertRow();

  const [isCreating, setIsCreating] = useState<boolean>(false);
  const handleCreate = async () => {
    setIsCreating(true);
    const { id } = await insertRow({
      name: `Untitled course ${teacher?.courses1.nodes.length}`,
    });
    setIsCreating(false);

    push({ pathname: "/courses/[id]", query: { id, edit: true } });
  };
      ...
      <div className="px-4 pt-14 pb-40">
        <div className="overflow-hidden sm:rounded-md">
          <ul className="divide-y divide-gray-200">
            {teacher?.courses1.nodes.length > 0 ? (
              teacher?.courses1.nodes.map((course: any, index) => (
                <li key={index}>
                ...
                description={
                  <p>
                    ...
                    <button
                      ...
                      className="disabled:opacity-50 relative inline-flex mt-4 items-center px-6 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                    >
                      {isCreating && (
                        <svg
